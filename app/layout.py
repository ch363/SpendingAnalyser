...
def render_dashboard(
    *,
    ...
    # Compute helper values and helpers early so we can render the budget card in the top-right
    today = date.today()
    ...
    # Lower-row budget cards removed; keep helper values only for AI suggestion context
    allowance = remaining / days_left  # retained for AI context (if used)
    avg_so_far = current_spend / max(1, day_of_month)
    target_pace = budget_value / max(1, days_in_month)
    on_track = avg_so_far <= target_pace
    month_complete = bool(getattr(context["budget"], "is_month_complete", False))
